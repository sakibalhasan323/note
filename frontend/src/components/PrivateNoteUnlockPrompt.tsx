...
import { Lock, KeyRound, Eye, EyeOff, AlertCircle, RefreshCw, CheckCircle2, ShieldCheck } from 'lucide-react';
...
export const PrivateNoteUnlockPrompt: React.FC<PrivateNoteUnlockPromptProps> = ({
  ...
  const { currentUser, reauthenticateWithGoogle, openAuth } = useAuth();
  ...
  // Guests must set a password first time. Guests' private notes are local-only.
  const needsPasswordSetup = isGuest && !storage.getGuestPrivatePasswordHash();

  if (isGuest && needsPasswordSetup) {
    // First-time guest: set a password for local private notes.
    ...
    return (
      <div className="w-full max-w-md mx-auto my-12 p-6 bg-white dark:bg-slate-850 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl text-center animate-in fade-in zoom-in-95 duration-200">
        ...
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Protect your private notes</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 mb-5">
          {targetNoteId
            ? 'Set a password for this private note. It is stored on this device only until you sign in with Google.'
            : 'Set a password for your private notes. They are saved on this device only until you sign in with Google.'}
        </p>

        {error && (
          <div className="mb-4 p-3 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 flex items-center justify-center gap-2 text-xs font-semibold text-red-600 dark:text-red-400">
        ...
        <form onSubmit={handleSetup} className="space-y-3">
          ...
          <div className="relative text-left">
            <input
              id="private-note-confirm-password"
              type={showPassword ? 'text' : 'password'}
          ...
          <button
            type="submit"
            id="private-note-set-password-btn"
        ...
        <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Want your private notes synced across devices?
          </p>
          <button
            type="button"
            onClick={openAuth}
            className="mt-1 text-xs text-indigo-600 dark:text-indigo-400 hover:underline font-medium cursor-pointer"
          >
            Sign in with Google
          </button>
        </div>
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      if (!isGuest) {
        const authResult = await reauthenticateWithGoogle();
        if (!authResult.success) {
          setError(authResult.error || 'Google re-authentication failed');
          return;
        }
      }
  ...
  return (
    <div className="w-full max-w-md mx-auto my-12 p-6 bg-white dark:bg-slate-850 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl text-center animate-in fade-in zoom-in-95 duration-200">
      ...
      <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 mb-6">
        {isResetMode
          ? isGuest
            ? 'Set a new password for your locally stored private notes.'
            : 'Private-note passwords cannot be recovered by email. Re-authenticate with Google and set a new password below.'
      ...
      {!isResetMode ? (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="pt-2">
            ...
            >
              {isGuest ? 'Change password' : 'Reset password'}
            </button>
          </div>
        </form>
      ) : (
        <form onSubmit={handleResetPassword} className="space-y-4">
          ...
          >
            ...
            <span>{isSubmitting ? 'Saving...' : isGuest ? 'Set new password' : 'Verify with Google & reset'}</span>
          </button>
