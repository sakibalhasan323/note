    ...
    >
      ...
      <div>
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          {!note.is_deleted && (
            <button type="button" title={note.is_private ? 'Make public' : 'Make private'} aria-label={note.is_private ? 'Make public' : 'Make private'} onClick={(e) => { e.stopPropagation(); updateNote(note.id, { is_private: !note.is_private }); }} className={`p-1.5 rounded-lg transition-colors ${note.is_private ? 'text-amber-500 opacity-100 hover:text-slate-700 dark:hover:text-slate-200' : 'text-slate-400 hover:text-amber-600 dark:hover:text-amber-300 opacity-0 group-hover:opacity-100 focus:opacity-100'}`}>
            ...
            <div className="relative" ref={moreMenuRef}>
              ...
              {showMoreMenu && (
                <div className="absolute right-0 bottom-8 z-30 w-36 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl py-1 shadow-xl text-xs">
                  ...
                  <button type="button" onClick={(e) => { e.stopPropagation(); openNoteEditor(note, true); setShowMoreMenu(false); }} className="hidden md:flex w-full px-3 py-2 text-left items-center gap-2 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300">
