...
import { Search, X, Sun, Moon, Menu, RefreshCw, User, Settings, LogOut, LogIn, LayoutGrid, List } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ onOpenSettings, viewMode, onViewModeChange }) => {
  const {
    searchQuery,
    setSearchQuery,
    theme,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2.5 select-none">
            {settings.logo ? (
              ...
              <div
                ...
                style={{ backgroundColor: settings.primary_color || '#c15f3c' }}
              >
        ...
        */}
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          <button type="button" onClick={() => onViewModeChange(viewMode === 'grid' ? 'list' : 'grid')} title={viewMode === 'grid' ? 'List view' : 'Grid view'} aria-label={viewMode === 'grid' ? 'Switch to list view' : 'Switch to grid view'} className="flex p-2 rounded-lg text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
          ...
          <button
            ...
            title="Reload notes"
            className="p-2 rounded-lg text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-800 active:scale-90 focus:outline-none transition-all disabled:cursor-wait disabled:opacity-70"
            aria-label="Reload notes"
          >
            <RefreshCw className={`w-4 h-4 transition-transform ${isReloading ? 'animate-spin text-indigo-600' : ''}`} />
          </button>
