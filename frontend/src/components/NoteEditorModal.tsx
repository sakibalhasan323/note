...
export const NoteEditorModal: React.FC = () => {
  ...
  const handleTogglePrivate = (enable: boolean) => {
    setIsPrivate(enable);
    if (enable) {
      updateNote(editingNote!.id, { is_private: true });
    } else {
      if (editingNote) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-stretch sm:items-center justify-center p-0 sm:p-6 overflow-y-auto">
      {/* Backdrop */}
      <div
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        className={`relative w-full sm:rounded-xl border shadow-xl z-10 flex flex-col overflow-hidden transition-colors animate-in zoom-in-95 fade-in duration-200 min-h-screen sm:min-h-0 ${isEditorExpanded ? 'sm:max-w-5xl' : 'sm:max-w-xl'}`}
        style={{
      ...
      >
        ...
        <div className="relative p-6 sm:p-5 pb-4 sm:flex sm:items-start sm:justify-between sm:gap-4">
          <input
            ...
            className="w-full min-w-0 px-1 pt-14 sm:pt-0 bg-transparent font-semibold text-lg sm:text-xl placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none"
            style={{ color: style.titleColor }}
          />

          <div className="absolute right-5 top-5 flex items-center gap-2 sm:static sm:shrink-0 sm:pl-2">
        ...
        <div className="flex-1 px-6 sm:px-5 py-5 sm:py-3">
          <textarea
            ref={textareaRef}
            ...
            rows={8}
            className="w-full h-full min-h-[55vh] sm:min-h-[160px] bg-transparent resize-none text-[13px] sm:text-sm placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none leading-relaxed max-h-[65vh]"
            style={{ color: style.bodyColor }}
          />
        </div>

        {/* Date metadata */}
        <div className="px-6 sm:px-5 py-3 flex items-center justify-between text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <div className="mt-auto p-4 sm:p-4 border-t border-black/5 dark:border-white/5 bg-black/2 dark:bg-white/2 flex items-center justify-between">
