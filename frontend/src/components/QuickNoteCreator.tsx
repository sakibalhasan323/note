...
export const QuickNoteCreator: React.FC<QuickNoteCreatorProps> = ({ makePrivate = false }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto mb-8 px-4" ref={containerRef}>
      ...
      >
        ...
        {isExpanded && makePrivate && (
          <div className="px-4 pb-3">
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Private-note password" className="w-full rounded-lg border border-amber-300 bg-amber-50/50 px-3 py-2 text-xs text-slate-700 outline-none focus:ring-2 focus:ring-amber-500/30 dark:border-amber-800 dark:bg-amber-950/20 dark:text-slate-200" />
          </div>
        )}
          ...
          <div className="flex items-center justify-between px-3 py-2 border-t border-slate-100 dark:border-slate-700/60 bg-slate-50/50 dark:bg-slate-800/40 rounded-b-xl">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={handleClose}
                disabled={makePrivate && !password.trim()}
                className="px-4 py-1.5 text-xs font-semibold rounded-lg bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white shadow-xs transition-colors flex items-center gap-1.5"
              >
