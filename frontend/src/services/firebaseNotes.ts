 ...
 * Stores notes in the user-nested path so realtime updates have one canonical source.
 */
export async function saveNoteToFirebase(note: Note): Promise<{ success: boolean; error?: string }> {
  ...
  // User-nested documents are the canonical storage location.
  try {
    ...
    const errorMsg = err?.message || 'Failed to save note in Firebase';
...
/**
 * Subscribe to the canonical user-nested collection for this user's notes.
 */
